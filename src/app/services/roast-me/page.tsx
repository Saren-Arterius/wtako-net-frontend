...
const RoastForm = observer(() => {
  ...
  useEffect(() => {
    ...
    return () => roastMeStore.disconnect();
  }, []);

  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!file) return;
    const text = [species, extra.trim()].filter(Boolean).join(" ") || "獸設";
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white/4 rounded-none sm:rounded-xl backdrop-blur-md p-6 -mx-6 sm:mx-0 border border-white/10">
        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="flex items-center justify-center h-96 rounded-lg border border-dashed border-white/20 bg-white/5 cursor-pointer hover:border-highlight/50 transition-colors overflow-hidden">
            <input
              type="file"
              accept="image/*"
              onChange={(e) => {
            ...
            {resultImage || roastMeStore.previewUrl ? (
              ...
              <span className="text-subtitle/40 text-sm">點選上傳圖片</span>
            )}
          </label>
      ...
      {/* Public Feed */}
      <div className="bg-white/4 rounded-none sm:rounded-xl backdrop-blur-md p-6 -mx-6 sm:mx-0 border border-white/10">
        <h2 className="text-xl text-highlight mb-4">最近被酸</h2>
        {roastMeStore.feed.length === 0 ? (
          <p className="text-subtitle/40 text-sm">還沒有被酸的獸設...</p>
        ) : (
          <div className="columns-2 sm:columns-3 lg:columns-4 gap-4">
            {roastMeStore.feed.map((entry) => (
              <div
                key={entry.image_sha512}
                onClick={() => setViewEntry(entry)}
                className="bg-white/5 rounded-lg overflow-hidden cursor-pointer hover:bg-white/10 transition-colors break-inside-avoid mb-4"
              >
                <FadeInImage
